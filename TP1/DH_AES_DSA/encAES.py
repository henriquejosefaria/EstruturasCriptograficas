...
class encAES():
    ...
    def generateSharedKey(self,sSharedSecret,eSharedSecret,salt=b"0"):
        kdf = PBKDF2HMAC(algorithm=hashes.SHA256(),length=(encAES.ENCRYPTION_KEY_SIZE + encAES.HMAC_KEY_SIZE),salt=salt,iterations=100,backend=self.backend)
    ...
    def sign(self,msg):
        return self.dsa_private_key.sign(msg,hashes.SHA256())

    def verifySign(self,msg,signature):
        self.dsa_public_key.verify(signature,msg,hashes.SHA256())
    ...
    def keyAgreementR(self,connection):
        ...
        # ephemeral
        e_pk = connection.recv(encAES.RCV_BYTES)
        ...
        # mac verification
        mac_and_sign = connection.recv(encAES.RCV_BYTES)
...
def gen_dsa_keys(backend):
    return  dsa.generate_private_key(key_size=1024,backend=backend)
